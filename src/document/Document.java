...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		char firstChar = Character.toLowerCase(cArray[0]);
		if(cArray.length > 1){
			for(int i = 0; i < cArray.length; i++){
				char x = Character.toLowerCase(cArray[i]);
				if(i != cArray.length - 1){
					...
					if(newSyllable && count >= 0 && (x == 'a' || x == 'i' || x == 'o' || x == 'u' || x == 'y')){
						count++;
					}
					else if(count == 0 && x == 'e'){
						count++;
		...
		if(cArray.length == 1 && (firstChar == 'a' || firstChar == 'e' || firstChar == 'i' || firstChar == 'o' || firstChar == 'u' || firstChar == 'y')){
			count++;
		}				
	...
	public double getFleschScore()
	{
		...
		double fleschScore = 206.835 - 1.015 * (1.0 * getNumWords() / getNumSentences()) - 84.6 * (1.0 * getNumSyllables() / getNumWords());
	    return fleschScore;
	}
